...
export class AiTestService {
  ...
  async testConnection() {
    ...
    try {
      ...
      if (error.response) {
        this.logger.warn(
          `FastAPI trả về lỗi với HTTP Status: ${error.response.status}`,
        );
        return {
          message:
            'Kết nối thành công nhưng FastAPI báo lỗi logic (VD: chưa có model weights)',
      ...
      return {
        message:
          'Không thể kết nối đến FastAPI. Hãy kiểm tra xem server uvicorn đã chạy chưa.',
        error: error.message,
      };
    }
  }
