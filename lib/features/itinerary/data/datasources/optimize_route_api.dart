...
class OptimizeRouteApi {
  static Future<
    ({List<ItineraryActivityEntity> optimized, List<String> reorderNotes})
  >
  optimizeDay(
    ...
    try {
      ...
      final response = await client.dio.post(
        '/itinerary/optimize-day',
        data: payload,
      );

      final data = response.data['optimized'] as List;
      final reorderNotes =
          (response.data['reorderNotes'] as List?)
              ?.map((e) => e.toString())
              .toList() ??
          [];
      ...
      if (data.isEmpty)
        return (
          optimized: <ItineraryActivityEntity>[],
          reorderNotes: <String>[],
        );
      ...
      final mappedOptimized = data.map((json) {
        ...
        return original.copyWith(
          ...
          transitDurationMinutes:
              json['transitDurationMinutes'] ?? original.transitDurationMinutes,
          transitDistanceKm: json['transitDistanceKm'] != null
              ? (json['transitDistanceKm'] as num).toDouble()
              : original.transitDistanceKm,
        );
      }).toList();
      ...
      if (e is DioException && e.response?.statusCode == 400) {
        ...
        // Backend giờ trả nguyên nhân thật (thay vì luôn báo "timeout") cho các lỗi
        // 400 khác — hiển thị nguyên message đó thay vì thông báo chung chung để
        // người dùng/dev biết chính xác vì sao không tối ưu được.
