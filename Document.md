...
## 8. Tích hợp Supabase & Auth

Service: Sử dụng SupabaseClient thông qua Dependency Injection (sl<SupabaseClient>()).

Authentication: Sử dụng Supabase Auth để quản lý JWT. Token phải được tự động đính kèm vào các yêu cầu API thông qua tầng network/.

DataSources: Tất cả các thao tác với bảng dữ liệu trên Supabase (như lưu lịch trình du lịch) phải nằm trong thư mục data/datasources/.
