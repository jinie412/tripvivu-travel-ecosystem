-- Scope GET /search/nearby to the itinerary's destination city so add/replace
-- suggestions never surface places from a different city, and add an index
-- so the filter stays fast.

create index if not exists idx_places_city_active_geo
  on travel.places (city_id, latitude, longitude)
  where is_approved = true
    and is_active = true
    and latitude is not null
    and longitude is not null;

drop function if exists travel.search_nearby_places(
  double precision,
  double precision,
  integer,
  uuid[],
...
as $$
  with candidates as (
    ...
    where p.is_approved = true
      ...
      -- Keep results scoped to the itinerary's destination city.
      and (
        coalesce(trim(p_city), '') = ''
        or travel.immutable_unaccent(coalesce(ci.name, ''))
          ilike '%' || travel.immutable_unaccent(trim(p_city)) || '%'
      )
  ),
  filtered as (
    select *
